...
function formatLocalEntryTime(entryTimeStr) {
  if (!entryTimeStr) return { time: '--', date: '' };
  try {
    const isoStr = entryTimeStr.includes('T')
      ? entryTimeStr
      : entryTimeStr.replace(' ', 'T').replace(' UTC', 'Z');
    const dateObj = new Date(isoStr);
    if (isNaN(dateObj.getTime())) {
      const parts = entryTimeStr.split(' ');
      return { time: parts[1] || entryTimeStr, date: parts[0] || '' };
    }
...
const CELLS = (liveStatus, symbol) => {
  const entryTimeFormatted = formatLocalEntryTime(liveStatus?.entry_time);
  return [
    { label: 'Total Wallet Balance', value: liveStatus?.wallet_balance ? parseFloat(liveStatus.wallet_balance).toFixed(2) : (liveStatus?.available_balance ? parseFloat(liveStatus.available_balance).toFixed(2) : '--'), unit: 'USDT', color: 'text-slate-800 dark:text-white font-extrabold' },
    { label: 'Available Balance', value: liveStatus?.available_balance ? parseFloat(liveStatus.available_balance).toFixed(2) : '--', unit: 'USDT', color: 'text-emerald-600 dark:text-emerald-400' },
    ...
    { label: 'Notional Value', value: liveStatus?.notional_value > 0 ? parseFloat(liveStatus.notional_value).toFixed(2) : (liveStatus?.entry_margin > 0 && (liveStatus?.leverage_at_entry || liveStatus?.leverage) ? (parseFloat(liveStatus.entry_margin) * (liveStatus.leverage_at_entry || liveStatus.leverage)).toFixed(2) : '--'), unit: 'USDT', color: 'text-purple-500 dark:text-purple-400' },
    { label: 'Entry Time (Local)', value: entryTimeFormatted.time, unit: entryTimeFormatted.date, color: 'text-slate-700 dark:text-slate-300' },
    { label: 'Leverage', value: liveStatus ? `${liveStatus.leverage_at_entry || liveStatus.leverage}×` : '--', unit: '', color: 'text-violet-500 dark:text-violet-400' },
