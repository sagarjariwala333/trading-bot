...
/**
 * Format a relative time string like "2s ago", "1m ago".
 */
...
const CELLS = (liveStatus, symbol, liveMarkPrice) => {
  // Prefer the always-fresh mark price from the WS payload; fall back to telemetry
  const displayMarkPrice = liveMarkPrice ?? liveStatus?.mark_price;
  ...
  return [
    ...
    { label: 'Mark Price', value: displayMarkPrice > 0 ? parseFloat(displayMarkPrice).toFixed(2) : '--', unit: '', color: 'text-cyan-500 dark:text-cyan-400', isMarkPrice: true },
...
function MetricCell({ label, value, unit, color, pulse }) {
  return (
    <div className={cn(
      "flex flex-col gap-2 p-4 hover:bg-slate-100/50 dark:hover:bg-white/[0.02] transition-colors rounded-xl border border-slate-200 dark:border-white/[0.04] bg-slate-100/30 dark:bg-slate-900/40",
...
export default function TelemetryCard({ liveStatus, symbol, isBotRunning, onStart, onStop, onClear, onCloseTrade, onReset, markPrice, markPriceUpdatedAt, dataStale }) {
  // Track mark price changes for the pulse animation
  const prevMarkPriceRef = useRef(null);
  const [isPulsing, setIsPulsing] = useState(false);

  useEffect(() => {
    if (markPrice != null && prevMarkPriceRef.current != null && markPrice !== prevMarkPriceRef.current) {
      setIsPulsing(true);
      const timer = setTimeout(() => setIsPulsing(false), 600);
      return () => clearTimeout(timer);
    }
    prevMarkPriceRef.current = markPrice;
  }, [markPrice]);

  const markPriceUnavailable = markPrice == null && (!liveStatus || !liveStatus.mark_price);
  const ageLabel = formatAge(markPriceUpdatedAt);
  ...
  return (
    <Card className="border-slate-200 dark:border-white/[0.06] bg-white dark:bg-[#0d1220]/80 backdrop-blur-xl shadow-md dark:shadow-xl dark:shadow-black/30 transition-colors duration-300">
      <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 py-4 border-b border-slate-200 dark:border-white/[0.05]">
        <div>
          <div className="flex items-center gap-3">
            ...
            {/* Staleness / connectivity badges */}
            {markPriceUnavailable && (
              <Badge variant="outline" className="px-2 py-0.5 text-[10px] font-bold border-red-500/50 text-red-400 bg-red-500/10 animate-pulse">
                ⚠ Live price unavailable
              </Badge>
            )}
            {!markPriceUnavailable && dataStale && (
              <Badge variant="outline" className="px-2 py-0.5 text-[10px] font-bold border-amber-500/50 text-amber-400 bg-amber-500/10">
                ⏳ Bot telemetry stale
              </Badge>
            )}
            {ageLabel && !markPriceUnavailable && (
              <span className="text-[10px] text-slate-400 font-medium">
                Price updated {ageLabel}
              </span>
            )}
          </div>
          <CardDescription>
      ...
      <CardContent className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-3 p-4">
        {CELLS(liveStatus, symbol, markPrice).map((cell) => (
          <MetricCell key={cell.label} {...cell} pulse={cell.isMarkPrice && isPulsing} />
        ))}
      </CardContent>
    </Card>
  );
